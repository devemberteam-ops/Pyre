...
/// Ask the user whether to create an EMBEDDED (card-only, hidden) or a
/// SHARED (reusable, visible) lorebook.
///
/// Returns:
///   true  → embedded (hidden=true; decluttered from the Lorebooks list)
///   false → shared (hidden=false; appears in Lorebooks + reusable)
///   null  → user cancelled
///
/// D1 requirement: ALWAYS ask at bind time; no silent default.
Future<bool?> askEmbeddedChoice(BuildContext context) {
  return showDialog<bool>(
    context: context,
    builder: (ctx) => AlertDialog(
      backgroundColor: EmberColors.bgPanel,
      title: const Text('Keep this lorebook…'),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: const [
          Text(
            'Embedded (card-only): lives with this card, hidden from the '
            'Lorebooks list. Travels inside the card on export.',
